...
class AddPricingToNestsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('nests', function (Blueprint $table) {
            $table->integer('database_limit')->default(1);
            $table->integer('allocation_limit')->default(1);
            $table->float('memory_monthly_cost')->default(.50);
            $table->float('disk_monthly_cost')->default(.50);
            $table->integer('cpu_limit')->default(0);
            $table->float('max_memory')->default(4096);
            $table->float('max_disk')->default(10240);
        });
    }
